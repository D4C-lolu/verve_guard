...
public class RoleServiceIntegrationTest extends BaseIntegrationTest {
    ...
    void shouldFailGetRoleWithNonExistentId() {
        assertThatThrownBy(() -> roleService.getRoleById("NONEXISTENT00000000000000"))
    ...
    void shouldFailAssignPermissionWithNonExistentRole() {
        ...
        assertThatThrownBy(() -> roleService.assignPermission("NONEXISTENT00000000000000", permission.getId()))
                .isInstanceOf(NotFoundException.class)
                .hasMessage("Role not found");
    }
    ...
    void shouldFailAssignPermissionWithNonExistentPermission() {
        ...
        assertThatThrownBy(() -> roleService.assignPermission(role.getId(), "NONEXISTENT00000000000000"))
                .isInstanceOf(NotFoundException.class)
    ...
    void shouldAssignPermissionsInBulkSuccessfully() {
        Role role = roleRepository.findByName("MERCHANT").orElseThrow();
        List<String> permissionIds = permissionRepository.findAll().stream()
    ...
    void shouldRevokePermissionsInBulkSuccessfully() {
        Role role = roleRepository.findByName("ADMIN").orElseThrow();
        List<String> permissionIds = rolePermissionRepository.findByRoleId(role.getId())
                .stream()
                .limit(3)
